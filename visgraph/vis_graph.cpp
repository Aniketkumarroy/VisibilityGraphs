...
VisGraph::VisGraph() : G_Ptr(nullptr), VG_Ptr(nullptr), graph_built(false) {}

void VisGraph::SingleThreadProcessing(const std::vector<Point>& points, int& batch_size){
    int s = points.size();
...
void VisGraph::MultiThreadProcessing(const std::vector<Point>& points, int& batch_size, int& threads){
...
void VisGraph::build(std::vector<std::vector<Point>> &polygons, std::pair<int,int> grid_resolution, int batch_size, int threads, bool status){
    Timer t;
...
void VisGraph::UpdateByADD(std::vector<std::vector<Point>> &polygons, int& batch_size, int& threads, bool tolerance){
    if(graph_built){
...
std::unordered_set<Point> VisGraph::find_visible_vertices(const Point& point, const Graph& graph){
